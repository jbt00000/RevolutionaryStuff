...
public static class PermaCache
{
    ...
    public static TRes FindOrCreate<TRes>(object key0, Func<TRes> creator, [CallerMemberName] string caller = null, [CallerLineNumber] int callerLine = 0)
        => FindOrCreatePrivate(Cache.CreateKey<TRes>(caller, callerLine, key0), creator);

    public static TRes FindOrCreate<TRes>(object key0, object key1, Func<TRes> creator, [CallerMemberName] string caller = null, [CallerLineNumber] int callerLine = 0)
        => FindOrCreatePrivate(Cache.CreateKey<TRes>(caller, callerLine, key0, key1), creator);

    public static TRes FindOrCreate<TRes>(object key0, object key1, object key2, Func<TRes> creator, [CallerMemberName] string caller = null, [CallerLineNumber] int callerLine = 0)
